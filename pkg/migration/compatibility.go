...
func RunCompatibilityTest(ctx context.Context, cfg *config.Config, log *logger.Logger) error {
	log.Info("================================================================================")
	...
	for pairIdx, pair := range cfg.DatabasePairs {
		...
		defer func() {
			...
			if err := coll.Drop(cleanupCtx); err != nil {
				log.Warnf("Failed to drop temporary collection %s: %v", tempCollName, err)
			}
		}()
		...
		testCases := []CompatibilityTestCase{
			// 1. ID Datatype Tests
			{Category: "ID Type Support", TestName: "ObjectID as _id", Document: bson.D{{"_id", primitive.NewObjectID()}}, ValueDesc: "standard BSON ObjectID"},
			{Category: "ID Type Support", TestName: "String as _id", Document: bson.D{{"_id", "test_string_id"}}, ValueDesc: "\"test_string_id\""},
			{Category: "ID Type Support", TestName: "Int64 (Long) as _id", Document: bson.D{{"_id", int64(1234567890)}}, ValueDesc: "1234567890 (int64)"},
			{Category: "ID Type Support", TestName: "Int32 as _id", Document: bson.D{{"_id", int32(12345)}}, ValueDesc: "12345 (int32)"},
			{Category: "ID Type Support", TestName: "Int as _id", Document: bson.D{{"_id", int(9876)}}, ValueDesc: "9876 (int)"},
			{Category: "ID Type Support", TestName: "Float64 (Double) as _id", Document: bson.D{{"_id", float64(3.14159)}}, ValueDesc: "3.14159 (double)"},
			{Category: "ID Type Support", TestName: "Boolean as _id", Document: bson.D{{"_id", true}}, ValueDesc: "true (boolean)"},
			{Category: "ID Type Support", TestName: "DateTime as _id", Document: bson.D{{"_id", primitive.NewDateTimeFromTime(time.Now())}}, ValueDesc: "current datetime"},
			{Category: "ID Type Support", TestName: "Binary (Bytes) as _id", Document: bson.D{{"_id", primitive.Binary{Subtype: 0, Data: []byte{1, 2, 3, 4}}}}, ValueDesc: "[1, 2, 3, 4] bytes"},
			{Category: "ID Type Support", TestName: "Array as _id", Document: bson.D{{"_id", bson.A{1, 2, 3}}}, ValueDesc: "[1, 2, 3] (array)"},
			{Category: "ID Type Support", TestName: "Empty String as _id", Document: bson.D{{"_id", ""}}, ValueDesc: "\"\""},
			{Category: "ID Type Support", TestName: "Empty Array as _id", Document: bson.D{{"_id", bson.A{}}}, ValueDesc: "[]"},
			{Category: "ID Type Support", TestName: "Empty Subdocument as _id", Document: bson.D{{"_id", bson.D{}}}, ValueDesc: "{}"},
			{Category: "ID Type Support", TestName: "Null as _id", Document: bson.D{{"_id", nil}}, ValueDesc: "null"},

			// 2. Key Length & Name Validation
			{Category: "Document Key Support", TestName: "Long Nested Field Name", Document: bson.D{{"_id", "test_long_field_key"}, {"nested", bson.D{{longFieldName, "value"}}}}, ValueDesc: "field name length = 1001 chars"},
			{Category: "Document Key Support", TestName: "Empty Field Name", Document: bson.D{{"_id", "test_empty_field_key"}, {"", "empty-value"}}, ValueDesc: "field name length = 0 chars"},
		}
		...
		// Print schema transformation configuration reference table
